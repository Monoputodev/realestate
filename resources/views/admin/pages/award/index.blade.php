            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Award</h4>
                            ...
                            <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($award->image)
                                            <img src="{{ asset('uploads/awards/' . $award->image) }}"
                                              alt="{{ $award->title }}" width="80">
                                            @else
                                            No Image
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <a class="btn btn-danger waves-effect btn-circle waves-light"
                                              href="{{ route('awards.inactive', $award->id) }}">
                                                <i class="fa fa-eye-slash"></i> </a>
                                            @else
                                            <a class="btn btn-success waves-effect btn-circle waves-light"
                                              href="{{ route('awards.active', $award->id) }}">
                                                <i class="fa fa-eye"></i> </a>
                                            @endif

                                            <a class="btn btn-primary waves-effect btn-circle waves-light"
                                              href="{{ route('awards.edit', $award->id) }}">
                                                <i class="fa fa-edit"></i> </a>
                                            <form hidden action="{{ route('awards.destroy', $award->id) }}"
                                              id="form{{ $award->id }}" method="get">
                                                @csrf
                                            </form>
                                            <button class="btn btn-danger waves-effect btn-circle waves-light"
                                              onclick="deleteItem({{ $award->id }});" type="button">
                                                <i class="fa fa-trash"></i> </button>
                                        </td>
                                        {{-- <td>
                                            <a href="{{ route('awards.edit', $award->id) }}" class="btn btn-sm
                                        btn-warning">Edit</a>
